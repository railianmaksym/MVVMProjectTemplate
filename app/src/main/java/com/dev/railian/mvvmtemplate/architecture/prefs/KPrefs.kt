...
/**
 * [KPrefs] is a small [SharedPreferences] wrapper that aims to provide a type-safe,
 ...
 * every get/set, or figure out how to obtain a Context object in utility methods. [KPrefs]
 ...
 * and generally there should only exist one KPrefs per backing preference file. The name
 * of preference file should be passed as the sole constructor parameter of [KPrefs].
 *
 * To create preference properties inside a [KPrefs], use any delegate of type [Pref], such
 ...
 *
 * Here is an example of a complete, simple KPrefs implementation:
 *
 * ```
 * object MyPrefs : KPrefs("my_prefs_file_name") {
 *
...
abstract class KPrefs {

    /* The [SharedPreferences] instance which serves as the core persistence mechanism */
    internal val prefs by lazy {
        ContextKeeper.appContext.getSharedPreferences("facedrive_preferences", Context.MODE_PRIVATE)
    }
    ...
    /**
     * Because [KPrefs] does not require its properties to be directly bound to the backing
    ...
    /**
     * Clears this KPrefs's backing [SharedPreferences] file. This is only guaranteed to clear
     * properties which have been delegated to an instance of the [Pref] class. Which properties
     * are reset is determined by the specific [KPrefs] implementation.
     ...
     * KPrefs instance for which this function is being overridden.
     */
    open fun keepBaseProps(): List<KMutableProperty0<out Any?>> = emptyList()

    /**
     * Clears this KPrefs's backing SharedPreferences file while preserving all properties
...
/**
 * Extend [Pref] to create concrete delegate classes for use in [KPrefs]. Subclasses are
 ...
 * when converting other [SharedPreferences] implementations to [KPrefs] and the existing
...
abstract class Pref<T>(private val defaultValue: T, val keyName: String? = null) :
    ...
    override fun getValue(thisRef: KPrefs, property: KProperty<*>): T = thisRef.prefs.getValue(
        keyName
            ?: property.name, defaultValue
    )

    override fun setValue(thisRef: KPrefs, property: KProperty<*>, value: T) =
...
/**
 * [Pref] delegate for [String] properties. May only be used in [KPrefs] implementations.
 *
 ...
 * to [KPrefs] and the required key name does not match the desired property name.
 */
...
/**
 * [Pref] delegate for [Boolean] properties. May only be used in [KPrefs] implementations.
 *
 * @param defaultValue The optional value returned for the property when no value has been set
...
/**
 * [Pref] delegate for [Int] properties. May only be used in [KPrefs] implementations.
 *
 * @param defaultValue The optional value returned for the property when no value has been set
...
/**
 * [Pref] delegate for [Long] properties. May only be used in [KPrefs] implementations.
 *
 * @param defaultValue The optional value returned for the property when no value has been set
...
/**
 * [Pref] delegate for [Float] properties. May only be used in [KPrefs] implementations.
 *
 * @param defaultValue The optional value returned for the property when no value has been set
...
/**
 * [Pref] delegate for [Int] color properties. May only be used in [KPrefs] implementations.
 *
 ...
 * serialized strings using Gson. May only be used in [KPrefs] implementations.
 *
 ...
 * as a serialized string using Gson. May only be used in [KPrefs] implementations.
 *
 * @param defaultValue (Optional) A default value to use until this property has been set.
 * @param keyName The optional key name under which the property value will be stored. Defaults
 * to the property name. This is useful when converting other [SharedPreferences] implementations
...
class GsonListPref<T>(
    ...
    keyName: String? = null
) : Pref<ArrayList<T>>(defaultValue, keyName) {

    private var cachedObject: ArrayList<T>? = null

    override fun onClear() {
        cachedObject = null
    }

    override fun SharedPreferences.getValue(key: String, default: ArrayList<T>): ArrayList<T> {
        if (cachedObject == null) {
            val type = TypeToken.getParameterized(ArrayList::class.java, klazz).type
            cachedObject = Gson().fromJson<ArrayList<T>>(getString(key, null), type)
        }
        return cachedObject ?: default
    }
 ...
 * string using Gson. May only be used in [KPrefs] implementations.
 *
 * @param defaultValue (Optional) A default value to use until this property has been set.
 * @param keyName The optional key name under which the property value will be stored. Defaults
 * to the property name. This is useful when converting other [SharedPreferences] implementations
